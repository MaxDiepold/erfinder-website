...
const agent = new https.Agent({ rejectUnauthorized: CFG.tlsVerify, keepAlive: true });
...
app.post("/api/login", (req, res) => {
  ...
  res.setHeader(
    "Set-Cookie",
    `lena_auth=${encodeURIComponent(signAuth(exp))}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${90 * 24 * 60 * 60}`
  );
  res.json({ ok: true });
});
...
function credentialsFrom(req) {
  const user = String(req.headers["x-fritz-user"] || CFG.envUser || "").trim();
  const password = String(req.headers["x-fritz-password"] || CFG.envPassword || "");
  if (!user || !password) {
    throw new Error("FRITZ!Box-Zugangsdaten fehlen. Öffne Einstellungen und speichere Benutzer + Passwort.");
  }
...
function pbkdf2Response(challenge, password) {
  ...
  if (!Number.isFinite(iter1) || !Number.isFinite(iter2) || !salt1.length || !salt2.length) {
    throw new Error("Ungültige FRITZ!Box-PBKDF2-Challenge.");
  }
  const h1 = crypto.pbkdf2Sync(Buffer.from(password, "utf8"), salt1, iter1, 32, "sha256");
  const h2 = crypto.pbkdf2Sync(h1, salt2, iter2, 32, "sha256");
  return `${p[4]}$${h2.toString("hex")}`;
}

async function getLoginChallenge() {
  const r = await http.get("/login_sid.lua?version=2");
  if (r.status < 200 || r.status >= 400) {
    throw new Error(`FRITZ!Box Login-Seite antwortet mit HTTP ${r.status}.`);
  }
  const challenge = xmlTag(r.data, "Challenge");
  const blockTime = Number(xmlTag(r.data, "BlockTime") || 0);
  if (!challenge) throw new Error("FRITZ!Box erreichbar, aber keine Login-Challenge erhalten.");
  return { challenge, blockTime };
}

async function getSid(creds) {
  const { challenge, blockTime } = await getLoginChallenge();

  // Nicht minutenlang warten. Safari/In-App-Browser brechen lange Requests sonst ab.
  if (blockTime > 0) {
    throw new Error(`FRITZ!Box hat weitere Anmeldeversuche für ${blockTime} Sekunden gesperrt. Bitte kurz warten und dann erneut testen.`);
  }

  const response = challenge.startsWith("2$")
    ? pbkdf2Response(challenge, creds.password)
    : md5Response(challenge, creds.password);

  const body = new URLSearchParams({ username: creds.user, response });
  const r = await http.post("/login_sid.lua?version=2", body.toString(), {
    headers: { "Content-Type": "application/x-www-form-urlencoded" }
  });

  if (r.status < 200 || r.status >= 400) {
    throw new Error(`FRITZ!Box-Anmeldung antwortet mit HTTP ${r.status}.`);
  }

  const sid = xmlTag(r.data, "SID");
  const nextBlock = Number(xmlTag(r.data, "BlockTime") || 0);

  if (!sid || sid === "0000000000000000") {
    const suffix = nextBlock > 0 ? ` Neuer Versuch frühestens in ${nextBlock} Sekunden.` : "";
    throw new Error(`FRITZ!Box lehnt die Anmeldung ab. Benutzername, Passwort oder die Berechtigung „Zugang aus dem Internet“ stimmen nicht.${suffix}`);
  }
  ...
  const walk = obj => {
    ...
    if (contains) {
      for (const key of candidates) {
        const v = obj[key];
        if ((typeof v === "string" || typeof v === "number") && String(v).length > 1) return String(v);
      }
    }
...
function findUidInText(text, targetIp) {
  try {
    const f = findUidInObject(JSON.parse(text), targetIp);
    if (f) return f;
  } catch {}

  const escaped = targetIp.replaceAll(".", "\\.");
  const around = text.match(new RegExp(`.{0,1600}${escaped}.{0,1600}`, "s"))?.[0] || "";
...
async function getTargetUid(sid) {
  ...
  throw new Error(`FRITZ!Box-Anmeldung erfolgreich, aber Gerät ${CFG.targetIp} wurde nicht in der Geräteliste gefunden.`);
}
...
function nightActiveNow() {
  ...
  const now = berlinNow();
  const start = hmToMin(state.nightStart);
  const end = hmToMin(state.nightEnd);
...
function publicState() {
  return {
    ok: true,
    ...state,
    nightActive: nightActiveNow(),
    runtimeReady: Boolean(runtimeCreds),
...
async function reconcileSchedule(force = false) {
  ...
  try {
    ...
    console.error("schedule:", state.lastError);
  } finally {
    schedulerBusy = false;
  }
}
...
app.post("/api/test", requireAuth, async (req, res) => {
  try {
    ...
    state.lastError = null;
    state.lastSuccess = new Date().toISOString();
    console.log(`FRITZ test OK for user ${creds.user}, target uid ${uid}`);
    res.json({ ok: true, uid, targetIp: CFG.targetIp });
  } catch (e) {
    state.lastError = e instanceof Error ? e.message : String(e);
    console.error("FRITZ test:", state.lastError);
...
app.post("/api/internet", requireAuth, async (req, res) => {
  ...
  try {
    ...
    state.lastError = e instanceof Error ? e.message : String(e);
    console.error("internet:", state.lastError);
    res.status(502).json({ ok: false, error: state.lastError });
  }
});
...
app.post("/api/timer", requireAuth, async (req, res) => {
  ...
  try {
    ...
    state.lastError = e instanceof Error ? e.message : String(e);
    console.error("timer:", state.lastError);
    res.status(502).json({ ok: false, error: state.lastError });
  }
});
...
app.post("/api/schedule", requireAuth, (req, res) => {
  ...
  state.days = Array.isArray(days)
    ? [...new Set(days.map(Number).filter(x => Number.isInteger(x) && x >= 0 && x <= 6))]
    : [];
  state.lastScheduleActive = null;
  setTimeout(() => reconcileSchedule(true), 0);
  res.json(publicState());
...
setInterval(async () => {
  ...
  if (state.timerEndsAt && Date.now() >= state.timerEndsAt) {
    ...
    try {
      ...
      state.lastError = e instanceof Error ? e.message : String(e);
      console.error("timer expiry:", state.lastError);
    } finally {
      schedulerBusy = false;
    }
    return;
  ...
  if (!state.timerEndsAt) await reconcileSchedule(false);
}, 5000);
